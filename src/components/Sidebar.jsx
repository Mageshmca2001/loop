        ...
        <nav className="mt-4">
          {menuItems.map((item) => (
            <div key={item.route}>
              ...
              {item.subItems && openDropdown === item.route && (
                <div className={`pl-8 ${isCollapsed ? 'hidden' : 'block'}`}>
                  {item.subItems.map((subItem) => (
                    <button
                      key={subItem.route}
                      className="w-full flex items-center px-4 py-2 hover:bg-gray-600 text-gray-300 transition-all"
                    >
                      {/* Add margin to space out the icon and label */}
                      <subItem.icon className="w-5 h-5 mr-4" />
                      <span>{subItem.label}</span>
                    </button>
                  ))}
                </div>
