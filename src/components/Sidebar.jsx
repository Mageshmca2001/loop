...
const Sidebar = ({ children }) => {   
  ...
  return (     
    <div className="flex h-screen " style={{ fontFamily: 'Poppins, sans-serif' }}>       
      {/* Sidebar */}       
      <div
        className={`fixed h-screen bg-blue-900 text-white shadow-2xl transition-all duration-300 ease-in-out
      ...
      >
        ...
        <div className="h-16 flex items-center justify-center border-b border-white">           
          {!isCollapsed ? (
            <h1 className="text-base font-bold tracking-widest"> BGT Dashboard</h1>           
          ) : (             
        ...
        <nav className="mt-6 h-full overflow-y-auto">
          {menuItems.map((section, index) => (
            <div key={index} className="mb-4">
              {!isCollapsed && (
                <h2
                  className="text-ml font-semibold text-white px-4 py-2 uppercase"
